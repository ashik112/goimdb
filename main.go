...
func CreateSolrFields(){
    ...
    go httpreq.CreateSolrFields("localhost",8983,"imdb",directory+"all_fields.json",doneTitles)
...
func UploadSolrData(){
    ...
    go httpreq.UploadDoc("localhost",8983,"imdb",directory+"title.principals.tsv",donePrincipals)
    <-donePrincipals   
    fmt.Println("Uploading Principals took ",time.Since(start))
    
    donePersons:=make(chan bool)
    go httpreq.UploadDoc("localhost",8983,"imdb",directory+"name.basics.tsv",donePersons)
    <-donePersons

    doneRatings:=make(chan bool)
    go httpreq.UploadDoc("localhost",8983,"imdb",directory+"title.ratings.tsv",doneRatings)
    doneCrew:=make(chan bool)   
    go httpreq.UploadDoc("localhost",8983,"imdb",directory+"title.crew.tsv",doneCrew)
    <- doneRatings
    <- doneCrew

    doneTitles:=make(chan bool)
    go httpreq.UploadDoc("localhost",8983,"imdb",directory+"title.basics.tsv",doneTitles)
    doneEpisodes:=make(chan bool)
    go httpreq.UploadDoc("localhost",8983,"imdb",directory+"title.episode.tsv",doneEpisodes)
    <- doneTitles
    <-doneEpisodes
}
...
func CMD(args []string){
    ...
    case contains(os.Args[1:],"update"):
         httpreq.DeleteAll("localhost",8983,"imdb")
         UploadSolrData()
